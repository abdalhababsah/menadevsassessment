...
export function Logo({ className, size = 'md' }: { className?: string; size?: 'sm' | 'md' | 'lg' | 'xl' }) {
  ...
  return (
    <motion.svg viewBox="0 0 100 100" fill="none" className={cn(sizes[size], className)} initial="initial" animate="animate">
      ...
      <motion.path d="M30 50L45 65L75 35" stroke="var(--verdant-500)" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }} />
    </motion.svg>
  );
}
